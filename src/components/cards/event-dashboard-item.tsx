...
export default function EventDashboardItem({ EventEntry }) {
  return (
    <div className="flex flex-col bg-white rounded-xl overflow-hidden drop-shadow-lg w-3/4 xl:w-2/3">
      ...
      <div className="flex flex-col w-full md:flex-row">
        <div className="flex justify-around p-4 font-bold leading-none text-gray-800 uppercase bg-gray-300 md:flex-col md:items-center md:justify-center md:w-1/4">
          <div className="md:text-3xl xl:text-4xl">{EventEntry.TimeOfEvent.getDate()}</div>
          <div className="md:text-lg xl:text-xl">{monthFormatter.format(EventEntry.TimeOfEvent)}</div>
          <div className="md:text-lg xl:text-xl">{EventEntry.TimeOfEvent.getFullYear()}</div>
        </div>
        <div className="flex flex-col gap-y-2 p-4 font-normal text-gray-800">
          <h1 className="text-xl xl:text-2xl font-bold tracking-tight text-gray-800">{EventEntry.Title}</h1>
          <p className="leading-normal text-ms xl:text-lg">{EventEntry.Description}</p>
          <div className="flex flex-row items-center text-gray-700 gap-2">
            ...
            <div className="text-ms xl:text-lg">{EventEntry.Location}</div>
          </div>
        </div>
      </div>
    </div>
